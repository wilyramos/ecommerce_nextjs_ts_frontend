...
export default function LayoutMinimal({ banner }: { banner: SliderBanner }) {
    ...
    return (
        <Link href={destUrl}
            ...
            <div className="relative z-10 flex flex-col items-center text-center gap-3 px-10 md:px-16 w-1/2" style={{ color: text }}>
                {subtitle && (
                ...
                {description && (
                    <p className="text-[13px] leading-relaxed" style={{ color: muted, opacity: loaded ? 1 : 0, transition: "opacity 0.55s ease 0.26s" }}>
                        {description}
                    </p>
                )}
