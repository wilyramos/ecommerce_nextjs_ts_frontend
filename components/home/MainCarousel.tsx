...
export default function MainCarousel({ products }: { products: ProductResponse[] }) {
    ...
    >
        {products.map((product) => (
            <div
                key={product._id}
                className="group relative flex flex-col md:flex-row items-center justify-between px-8 md:px-20 py-12 md:py-20 h-auto md:h-[420px] bg-gradient-to-br from-neutral-50 to-white rounded-2xl shadow-sm"
            >
                {/* Text Section */}
                <article className="z-10 w-full md:w-1/2 space-y-4">
                    ...
                    <h2 className="text-4xl md:text-5xl font-extrabold tracking-tight text-neutral-900 leading-[1.1] drop-shadow-sm">
                        {product.nombre}
                    </h2>
                ...
                {/* Image Section */}
                <div className="relative w-full md:w-1/2 h-[260px] md:h-full flex justify-center mt-8 md:mt-0">
