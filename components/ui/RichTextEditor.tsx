...
import Toolbar from "@/components/admin/products/Toolbar"; // Mantenemos tu toolbar globalizado
import EditorTheme from "@/components/form/editor/EditorTheme";

const editorConfig: InitialConfigType = {
    namespace: "EcommerceGlobalRichEditor",
    theme: EditorTheme,
...
function MediaInsertionPlugin({ urlsToInsert, onClearUrls }: { urlsToInsert: string[]; onClearUrls: () => void }) {
    const [editor] = useLexicalComposerContext();
...
interface RichTextEditorProps {
    initialHTML?: string;
    name: string; // Atributo name dinámico para interceptar en FormData corporativos
    placeholder?: string;
...
export default function RichTextEditor({ 
    ...
    return (
        <div className="w-full border border-border/60 rounded-lg overflow-hidden bg-background text-foreground shadow-sm">
            {/* Input oculto reactivo mapeado dinámicamente según la necesidad del módulo */}
            <input type="hidden" name={name} value={html} />

            <LexicalComposer initialConfig={editorConfig}>
                <div className="flex items-center justify-between border-b border-border/40 bg-muted/20 w-full">
                    <Toolbar 
                ...
                {isHTMLMode ? (
                    <textarea
                        className="w-full h-[320px] p-4 font-mono text-xs bg-muted/10 border-t border-border/40 text-foreground outline-none resize-none focus:ring-1 focus:ring-primary"
                        value={html}
