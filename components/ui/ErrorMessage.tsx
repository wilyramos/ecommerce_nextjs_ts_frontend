...
export default function ErrorMessage({ children, variant = "error", mode = "inline", onDismiss }: ErrorMessageProps) {
  const { bg, border, color } = CONFIG[variant];

  if (mode === "inline") {
    return (
      <div className="mt-1 flex items-center gap-1.5 text-xs font-medium" style={{ color }}>
        <span>{children}</span>
      </div>
    );
  }
  ...
  return (
    <div role="alert" className="flex items-start justify-between gap-3 rounded-xl border px-4 py-3 text-xs shadow-sm transition-colors" style={{ background: bg, borderColor: border, color }}>
      <div className="flex items-start gap-2">
        <div className="leading-relaxed">{children}</div>
      </div>
      {onDismiss && (
        <button onClick={onDismiss} aria-label="Cerrar" className="shrink-0 rounded-md p-1 transition-opacity hover:opacity-60 focus:outline-none focus:ring-2" style={{ color, outlineColor: "var(--ring)" }}>
          <X className="h-4 w-4" />
        </button>
      )}
    </div>
